...
When(
  'I check JSReports using the following report filters',
  { timeout: 60 * 1000 },
...
When(
  'I check JSReports for the HIV Dashboard named {string} using the following report filters',
  async function (reportName, table) {
    const params = {};
    table.hashes().forEach((hash) => {
      params[hash.field] = hash.value;
    });
...
Then(
  'there should be a row identified by {string} of {string} with the following fields and values',
...
Then('there should NOT be a row identified by {string} of {string}', function (
  field,
  value,
) {
  const row = this.output.rows.find((r) => r[field] === value);

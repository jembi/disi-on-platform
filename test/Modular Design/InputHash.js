    ...
    if (INPUT_DATA_LAST_ROW > 0) {
      ...
      for (var j = 0; j < INPUT_DATA_LAST_ROW; j++) {
        ...
        if (INPUT_DATA.values[j][2] == "R" || INPUT_DATA.values[j][2] == "NR") {
          ...
          const MUST_BE_REPORTED_ON =
            INPUT_DATA.values[j][2] == "R" ? true : false;
          ...
          const DYNAMIC_MRN_POSTFIX =
          BASE.DAY_OF_YEAR_POSTFIX + "-" + BASE.HH_MM_SS;
          ...
          for (var i = 0; i <= NUMBER_OF_ENCOUNTERS; i++) {
            ...
            ENCOUNTERS.Data.EntryToCare.CLIENT_UNIQUE_ID_ASSIGNED_AT_ENROLLMENT =
              base
                .getInputFieldValue(INPUT_DATA.values, ROW, i, 32, false)
                .trim() != ""
                ? base.getInputFieldValue(
                    INPUT_DATA.values,
                    ROW,
                    i,
                    32,
      ...
      console.log(
        "Google Drive input and expected outcome data cannot be empty!"
      );
    }
  }
}
